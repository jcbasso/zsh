...
# Picked from Derek Taylor colorscripts repo: https://gitlab.com/dwt1/shell-color-scripts/-/blob/master/colorscript.sh
# Simple CLI for shell-color-scripts

DIR_COLORSCRIPTS="$HOME/.zsh/colorscripts/colorscripts"
...
function _random() {
    ...
    random_colorscript="$(echo  "${list_colorscripts}" | sed -n ${random_index}p \
        | tr -d ' ' | tr '\t' ' ' | cut -d ' ' -f 2)"
    # echo "Random: ${RANDOM}"
    # echo "Total: ${length_colorscripts}"
    # echo "Index: ${random_index}"
    # echo "Colorscript: ${random_colorscript}"
